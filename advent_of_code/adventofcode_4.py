...
# description https://adventofcode.com/2020/day/2#part2

""" Advent of Code exercise 3 """
...
def read_in(data):
    ...
    # split input string on every new line

    data_list = data.split('\n')
    print("Number of all elements: {}.".format(len(data_list)))

    # deleting empty elements

    clean_list = [x for x in data_list if x]
    print("There are {} possible passwords.".format(len(clean_list)))

    # divide string element into components
    # lower range, upper range, the letter, the password
    ...
    for i in clean_list:
        first_split = i.split('-')
        position_x.append(first_split[0])
        second_split = first_split[1].split(' ')
        position_y.append(second_split[0])
        letters.append(second_split[1][0])
        passwords.append(second_split[2])
    ...
    while n < len(passwords):
        x = int(position_x[n])
        y = int(position_y[n])
        pswd = passwords[n]
        letter = letters[n]

        # if letter at one of the correct locations count as valid
        if pswd[x-1] == letter or pswd[y-1] == letter:
            valid_count += 1
        # if letter at both of the correct location count as invalid
        if pswd[x-1] == letter and pswd[y-1] == letter:
            valid_count -=1
        n += 1
