...
def read_in_data(data):
    ...
    # making sure there are not new lines and
    # passport data is all on one line

    passports_processed = []
    for i in clean_passports:
        if '\n' in i:

            one_line = i.replace('\n', ' ')
            passports_processed.append(one_line + ' ')  # one space added to end of passport (index error fix)
        else:

            passports_processed.append(i + ' ')

    return passports_processed


def validate_fields(data):

    passed = 0  # number of validated passports
    failed = 0  # number of failed passports
    ...
    while n < len(passwords_list):
        password = passwords_list[n]
        values = []
        for i in fields:
            start_field = password.find(i+':')
            end_field = password[start_field:].find(' ')
            field_value = password[start_field+4:start_field + end_field]
            value = field_value
            print(value)
            # validate values
            ...
            # Birth Year
            if i == fields[0]:
                if len(value) == 4 and (int(value) >= 1920 <= 2002):
                    check += 1
            # Issue Year
            if i == fields[1]:
                if len(value) == 4 and int(value) >= 2010 <= 2020:
                    check += 1
            # Expiration Year
            if i == fields[2]:
                if len(value) == 4 and int(value) >= 2020 <= 2030:
                    check += 1
            # Height
            if i == fields[3]:
                if 'cm' in value and int(value[:-2]) >= 150 <= 195:
                    check += 1
                if 'in' in value and int(value[:-2]) >= 59 <= 76:
                    check += 1
            # Hair Color
            if i == fields[4]:
            ...
            # Country ID
            # if i == fields[7]:
            #     if password.find('cid') > 0:
            #         #check += 1  # opted out of validating 'cid'
            #         pass
            #     else:
            #         pass
    ...
    print('\nAll Passports: {} \n\n\tValid: {} \n\tInvalid:{}'.format(len(passwords_list),
...
pass_data = """eyr:2029 pid:157374862
